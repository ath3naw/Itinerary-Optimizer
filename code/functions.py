...
def plot_path(df, order, title="NYC Museums"):
    ...
    lim_x = min(df.loc[start_idx, "easting"], df.loc[end_idx, "easting"]) - 500, max(df.loc[start_idx, "easting"], df.loc[end_idx, "easting"]) + 500
    lim_y = min(df.loc[start_idx, "northing"], df.loc[end_idx, "northing"]) - 1000, max(df.loc[start_idx, "northing"], df.loc[end_idx, "northing"]) + 1000
    plt.xlim(lim_x)
    plt.ylim(lim_y)
